...
const GroupIcon = () => (
  <svg 
    ...
    className="text-[#4fb26e]" // Apply the color using Tailwind's text color utility
  >
...
export default function GroupTable() {
  ...
  const getGroup = useCallback(async () => {
    ...
    try {
      const data: GroupResponse = await HTTPGetWithToken(`${BASEURL}/group/member/${userId}`, token);
      if (data.code === 200) {
  ...
  const handleViewGroup = (groupId: number) => {
    router.push(`/group-details?id=${groupId}`); // small 'id'
  };
              ...
              group.map((groupItem, index) => (
                ...
                >
                  <TableCell className="whitespace-nowrap px-6 py-4">
                    ...
                    <div className="flex h-10 w-10 flex-shrink-0 items-center justify-center rounded-full bg-[#e6f9ee] text-blue-600 dark:bg-blue-900 dark:text-blue-200">
                        <GroupIcon />
                      </div>
                      <div className="ml-4">
                  ...
                  <TableCell className="whitespace-nowrap px-6 py-4">
                    <div className="text-sm text-gray-900 dark:text-white">
                      {groupItem.next_payment || 'N/A'}
                    </div>
                  </TableCell>
                  <TableCell className="whitespace-nowrap px-6 py-4">
                    <div className="text-sm text-gray-900 dark:text-white">
                      {groupItem.start_date}
                    </div>
                  </TableCell>
                  <TableCell className="whitespace-nowrap px-6 py-4">
                    <div className="text-sm font-medium text-gray-900 dark:text-white">
                      ₦{groupItem.group_value ? groupItem.group_value.toLocaleString('en-US', {
                  ...
                  <TableCell className="whitespace-nowrap px-6 py-4 text-right text-sm font-medium">
                    <Button
                      variant="outline"
                      size="sm"
                      onClick={() => handleViewGroup(groupItem.id)}
